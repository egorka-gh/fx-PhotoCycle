package com.photodispatcher.model.dao{
	...
	public class LayerDAO extends BaseDAO{
		...
		public function findAll(silent:Boolean=false, forEdit:Boolean=false):ArrayCollection{
			var res:Array=findAllArray(silent,forEdit);
			return new ArrayCollection(res);
		}
		
		public function findAllArray(silent:Boolean=false, forEdit:Boolean=false):Array{
			var sql:String='SELECT s.id, s.name'+
				' FROM config.layer s';
			if(forEdit) sql+=' WHERE s.id NOT IN (0,1)';
			sql+=' ORDER BY s.name';
		...
		override public function save(item:Object):void{
			...
			if(!it) return;
			if(it.id==0 || it.id==1)  return; //predefined
			if (it.id==-1){
				create(it);
			}else{
				update(it);
