...
package com.google.zxing.oned
{
	...
	public final class Code128Writer extends UPCEANWriter {
		...
		public function draw(contents:String, height:int, step:int=1, color:int=0):Bitmap{
			...
			for (var i:int=0;i<result.getWidth();i++) resultBits[i] = result._get(i,0);
			
			// map the result onto bitmap
			var bmpd:BitmapData = new BitmapData(result.getWidth(), height, false, 0x009900);
		...
		private function drawMatrix(contents:String, height:int, step:int=1):BitMatrix{
			...
			//var outputX:int = (UPCEANReader.START_END_PATTERN.length-1)*step;
			var outputX:int = int((outputWidth -inputWidth*step)/2);
			for (var inputX:int = 0; inputX < inputWidth; inputX++, outputX += step) {
				if (code[inputX] == 1) 
				{
					output.setRegion(outputX, 0, step, outputHeight);
				}
			}
			return output;
										 ...
										 hints:HashTable = null):Object // either BitMatrix or String 
		{
			...
			while (position < length) {
				...
				/*force CODE_CODE_B
				var newCodeSet:int= CODE_CODE_B;
				*/
